...
///    it is tapped.
abstract class XcPopupMenuEntry<T> extends StatefulWidget {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const XcPopupMenuEntry({ Key key }) : super(key: key);
...
class XcPopupMenuDivider extends XcPopupMenuEntry<Null> {
  /// Creates a horizontal divider for a popup menu.
  ///
  /// By default, the divider has a height of 16 logical pixels.
  const XcPopupMenuDivider({ Key key, this.height = _kMenuDividerHeight }) : super(key: key);
...
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class XcPopupMenuItem<T> extends XcPopupMenuEntry<T> {
  /// Creates an item for a popup menu.
  ///
  /// By default, the item is [enabled].
  ///
  /// The `enabled` and `height` arguments must not be null.
  const XcPopupMenuItem({
    Key key,
    this.value,
    this.enabled = true,
  ...
  @override
  XcPopupMenuItemState<T, XcPopupMenuItem<T>> createState() => XcPopupMenuItemState<T, XcPopupMenuItem<T>>();
}
...
class XcPopupMenuItemState<T, W extends XcPopupMenuItem<T>> extends State<W> {
...
///     }
///   },
///   itemBuilder: (BuildContext context) => <XcPopupMenuEntry<Commands>>[
...
///  * [PopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class XcCheckedPopupMenuItem<T> extends XcPopupMenuItem<T> {
  ...
  @override
  _XcCheckedPopupMenuItemState<T> createState() => _XcCheckedPopupMenuItemState<T>();
}

class _XcCheckedPopupMenuItemState<T> extends XcPopupMenuItemState<T, XcCheckedPopupMenuItem<T>> with SingleTickerProviderStateMixin {
...
class _XcPopupMenu<T> extends StatelessWidget {
  const _XcPopupMenu({
    Key key,
    this.route,
    this.semanticLabel,
  ...
  final _XcPopupMenuRoute<T> route;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget child = ConstrainedBox(
      constraints: BoxConstraints(
        ...
        maxWidth: /*_kMenuMaxWidth*/MediaQuery.of(context).size.width,
      ),
...
class _XcPopupMenuRouteLayout extends SingleChildLayoutDelegate {
  _XcPopupMenuRouteLayout(this.position, this.itemSizes, this.selectedItemIndex, this.textDirection);
  ...
  @override
  bool shouldRelayout(_XcPopupMenuRouteLayout oldDelegate) {
...
class _XcPopupMenuRoute<T> extends PopupRoute<T> {
  _XcPopupMenuRoute({
    this.position,
    this.items,
    this.initialValue,
  ...
  final RelativeRect position;
  final List<XcPopupMenuEntry<T>> items;
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
    ...
    Widget menu = _XcPopupMenu<T>(route: this, semanticLabel: semanticLabel);
      ...
      child: Builder(
        builder: (BuildContext context) {
          return CustomSingleChildLayout(
            delegate: _XcPopupMenuRouteLayout(
              position,
              itemSizes,
              selectedItemIndex,
...
/// [XcPopupMenuEntry.represents] method returns true for `initialValue`, and then
/// summing the values of [XcPopupMenuEntry.height] for all the preceding widgets
/// in the list.
///
...
///    semantics.
Future<T> xcShowMenu<T>({
  @required BuildContext context,
  @required RelativeRect position,
  @required List<XcPopupMenuEntry<T>> items,
  T initialValue,
  double elevation,
  ...
  return Navigator.of(context, rootNavigator: useRootNavigator).push(_XcPopupMenuRoute<T>(
    position: position,
    items: items,
...
///   itemBuilder: (BuildContext context) => <XcPopupMenuEntry<WhyFarther>>[
///     const PopupMenuItem<WhyFarther>(
///       value: WhyFarther.harder,
///       child: Text('Working a lot harder'),
///     ),
...
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
class XcPopupMenuButton<T> extends StatefulWidget {
  /// Creates a button that shows a popup menu.
  ///
  /// The [itemBuilder] argument must not be null.
  const XcPopupMenuButton({
    Key key,
  ...
  final XcPopupMenuItemBuilder<T> itemBuilder;
  ...
  /// called instead.
  final XcPopupMenuItemSelected<T> onSelected;

  /// Called when the user dismisses the popup menu without selecting an item.
  ///
  /// If the user selects a value, [onSelected] is called instead.
  final XcPopupMenuCanceled onCanceled;
  ...
  @override
  XcPopupMenuButtonState<T> createState() => XcPopupMenuButtonState<T>();
}
...
class XcPopupMenuButtonState<T> extends State<XcPopupMenuButton<T>> {
  ...
  void showButtonMenu() {
    ...
    final List<XcPopupMenuEntry<T>> items = widget.itemBuilder(context);
