...
import 'package:fast_ai/widgets/bottom_line.dart';
...
import 'ai_refresh_view.dart';

//list view 下拉刷新，列表加载更多
class AiRefreshListView<T extends IViewModel> extends StatelessWidget {
  final FreshListViewViewModel<T> model;
  ...
  Widget build(BuildContext context) {
    Widget loading = loadingView??MatchLoading(
      isListView:false,
      count: 10,
    );
    // loading = Container(
    ...
    Widget bottomWidget;
    bottomWidget = this.bottomWidget ?? Container(
      height: 100,
      child:  BottomLine(),
    );
    // Container(
...
class FreshListViewViewModel<T extends IViewModel>
    extends LoadMoreViewModel<T> {
  FreshListViewViewModel.create({
