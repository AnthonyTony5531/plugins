...
class AiLauncherPageViewModel extends StatefulViewModel<AiLaucherPageStyle>{

  AiLauncherPageViewModel();

  //参数传给view时候，在这初始化统一入口初始化，自己要啥扩展参数。注意。Page和Dialog.page参数来源于View的构造函数，
  AiLauncherPageViewModel.create();
  //更新统一走这个方法
  void update(){
 ...
 //initState时候同步调用，同步方法。这里不能做io, cache读写.查询等耗时操作。
  ...
  void didModelUpdate(IViewModel o, IViewModel n){
    AiLauncherPageViewModel  oldeViewModel = o as AiLauncherPageViewModel;
    AiLauncherPageViewModel  newViewModel = n as AiLauncherPageViewModel;
  }
  ...
  @override
  List<AiLaucherPageStyle> get styleList => [ AiLaucherPageStyle()];
}
