...
import 'package:fast_ai/widgets/likebutton/painter/bubbles_painter.dart';
import 'package:fast_ai/widgets/likebutton/painter/circle_painter.dart';
import 'package:fast_ai/widgets/likebutton/utils/like_button_model.dart';
import 'package:fast_ai/widgets/likebutton/utils/like_button_typedef.dart';
import 'package:fast_ai/widgets/likebutton/utils/like_button_util.dart';
...
class LikeButton extends StatefulWidget {
  const LikeButton(
      ...
      this.frequencyLimit,
      this.enabledOnTapDebounce = false,
      })
      : assert(size != null),
  ...
  /// tap call back of like button
  final LikeButtonTapUpCallback onTapUp;

  final LikeButtonTapDownCallback onTapDown;
  ///whether it is liked
  final bool isLiked;
  ...
  ///frequencyLimit for debounce tap
  final FrequencyLimit frequencyLimit;

  ///whether enable debounce tap
  final bool enabledOnTapDebounce;
...
class LikeButtonState extends State<LikeButton> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _frequencyLimit = widget.frequencyLimit ?? FrequencyLimit();
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTapDown: onTapDown,
      onTap: widget.enabledOnTapDebounce ? debounceOnTap : onTap,
      onTapUp: onTapUp,
      child: result,
    );
  }
  ...
  void debounceOnTap() {
    _frequencyLimit.onTask(task: onTap);
  }
  ...
  void onTap() {
    ...
    if (config.isNotConnect) {
      showToas('${config.langMap['baseLang']['networkError']['networkNotStrong']}');
      return;
    }
  ...
  void onTapUp(TapUpDetails details){
    ...
    if (widget.onTapUp != null) {
      widget.onTapUp(details).then((bool isLiked) {
//        _handleIsLikeChanged(isLiked);
      });
    } else {
//      _handleIsLikeChanged(!(_isLiked ?? true));
