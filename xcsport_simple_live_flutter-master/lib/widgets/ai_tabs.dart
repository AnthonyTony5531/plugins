    ...
    if (icon == null) {
      ...
      label = Column(
        ...
        children: <Widget>[
          Container(
            child: icon,
            margin: isHideIndicator?EdgeInsets.zero:const EdgeInsets.only(bottom: 0.0),
          ),
          _buildLabelText(),
        ],
      );
...
class AiTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  final Color unselectedBgCountColor;
  ...
  @override
  _AITabBarState createState() => _AITabBarState();
}

class _AITabBarState extends State<AiTabBar> {
  ...
  Decoration get _indicator {
    ...
    Color color = widget.indicatorColor ?? Theme.of(context).colorScheme.secondary;
  ...
  @override
  void didUpdateWidget(AiTabBar oldWidget) {
  ...
  /// 修复未选择状态下count颜色设置无效果bug
  TextStyle countDisplayStyle(int i) {
    Color displayColor = widget.unselectedCountColor ?? widget.countColor;
    if(i == _currentIndex){
      displayColor = widget.countColor;
    }

    return TextStyle(fontSize: 9).copyWith(color: displayColor ?? config.customStyle.themeIncludeFontColor);
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.tabs.length; i += 1) {
      if(!widget.showCount){
        ...
        String count;
        if(widget.tabs[i] is AiTab)
          count = (widget.tabs[i] as AiTab).count;
              ...
              child:  Row(
                ...
                children: <Widget>[
                  count == null ? Container() : Container(
                      padding: widget.isHideIndicator?EdgeInsets.zero:EdgeInsets.only(bottom: 15),
                      child: ClipOval(
                        child: Container(
                          constraints: BoxConstraints(minWidth: 15), height: 15,
                          alignment: Alignment(0, 0),
                          color:  Colors.transparent,
                          child: Text(count,style: TextStyle(color: Colors.transparent),),
                        ),
                      )
                  ),
                  widget.tabs[i],
                  count == null ? Container() : Container(
                    ...
                    child: ClipOval(
                      child: Container(
                        constraints: BoxConstraints(minWidth: 15), height: 15,
                        alignment: Alignment(0, 0),
                        color:  i != _currentIndex ? Colors.transparent : (widget.countBgColor ?? config.skin.colors.attentionColor1),
                        child: Text(count,style: countDisplayStyle(i),),
                      ),
                    )
                  )
                ],
    ...
    for (int index = 0; index < tabCount; index += 1) {
      wrappedTabs[index] = GestureDetector(
        ...
        child: Container(
          color: Colors.transparent,
          child: Padding(
            padding:widget.isHideIndicator?EdgeInsets.zero: EdgeInsets.only(bottom: widget.indicatorWeight),
            child: Stack(
              children: <Widget>[
                wrappedTabs[index],
                Semantics(
    ...
    Widget tabBar = CustomPaint(
      painter: widget.isHideIndicator?EmptyCustomPainter():_indicatorPainter,
      child: _TabStyle(
    ...
    return Container(
      padding: widget.isHideIndicator?EdgeInsets.zero:EdgeInsets.only( bottom: widget.paddingBottom),
      child: tabBar);
  }
}
...
class TabPageSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color fixSelectedColor = selectedColor ?? config.customStyle.themeColor;
...
class _UnderlinePainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    final Paint paint = borderSide.toPaint()..strokeCap = StrokeCap.round..style = PaintingStyle.fill;
