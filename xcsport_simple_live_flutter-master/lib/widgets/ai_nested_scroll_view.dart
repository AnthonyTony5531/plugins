...
class _NestedScrollViewState extends State<AiNestedScrollView> {
  ...
  void initState() {
    ...
    _coordinator.alwayScroller = widget.alwayScroll ?? false;
  }
  ...
  @override
  void didUpdateWidget(AiNestedScrollView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.controller != widget.controller){
      _coordinator.setParent(widget.controller);
    }

    if(oldWidget.alwayScroll != widget.alwayScroll){
      _coordinator.alwayScroller = widget.alwayScroll ?? false;
    }
  }
...
class _NestedScrollCoordinator implements ScrollActivityDelegate, ScrollHoldController {
  ...
  set alwayScroller(bool isCan){
    this._alwayScroll = isCan ?? false;
  }
  ...
  _NestedScrollMetrics _getMetrics(_NestedScrollPosition innerPosition, double velocity) {
    ...
    try{
      ...
      debugPrint('AiNestedScrollView => e : ${e.toString()}  stack : ${stack.toString()}');
      return _NestedScrollMetrics(
        minScrollExtent: _outerPosition.minScrollExtent,
  ...
  Drag drag(DragStartDetails details, VoidCallback dragCancelCallback) {
    ...
    if(_alwayScroll){
      beginActivity(
        _CanDragScrollActivity(_outerPosition, drag),
            (_NestedScrollPosition position) => _CanDragScrollActivity(position, drag),
      );
    }else{
      beginActivity(
...
class _CanDragScrollActivity extends DragScrollActivity{

  _CanDragScrollActivity(ScrollActivityDelegate delegate, ScrollDragController controller) : super(delegate, controller);
