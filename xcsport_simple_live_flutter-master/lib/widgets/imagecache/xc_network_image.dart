...
///
/// 修改自flutter sdk NetworkImage，加入3级存缓存的磁盘缓存，并且加入了网络图片的预下载功能
/// xcNetworkImage.NetworkImage(url).preLoad();
///
@immutable
class NetworkImage extends image_provider.ImageProvider<image_provider.NetworkImage> implements image_provider.NetworkImage {
  ...
  Future<ui.Codec> _loadAsync(
    ...
    try {
      ...
      if(cacheBytes != null) {
        var codeUi = PaintingBinding.instance.instantiateImageCodec(cacheBytes);
        return codeUi;
      }
      /// 新增代码块end
  ...
  Future<void> preLoad() async {
    try {
      /// 新增代码块start
      /// 从缓存目录中查找图片是否存在
      final Uint8List cacheBytes = await _cacheFileImage.getFileBytes(url);
      if(cacheBytes != null) {
        return ;
      }
      /// 新增代码块end
      ...
      /// 新增代码块start
      /// 将下载的图片数据保存到指定缓存文件中
      await _cacheFileImage.saveBytesToFile(url, bytes);
      /// 新增代码块end
