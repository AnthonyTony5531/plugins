...
import 'package:fast_ai/widgets/ai_html/image_properties.dart';
...
class AiHtmlRichTextParser extends StatelessWidget {
  AiHtmlRichTextParser({
    this.shrinkToFit,
    this.onLinkTap,
  ...
  final String Function(String url) onParserImgUrl;
    ...
    // OTHER ELEMENT NODES
      ...
      if (_supportedStyleElements.contains(node.localName)) {
        ...
        if(node.attributes['style'] != null) {
          childStyle = childStyle.merge(parseStyle(node.attributes['style']));
        }
            ...
            if (showImages) {
              if (node.attributes['src'] != null) {
                ...
                if (node.attributes['src'].startsWith("data:image") &&
                  ...
                  parseContext.rootWidgetList.add(GestureDetector(
                    child: Image.network(
                      this.onParserImgUrl == null ? node.attributes['src']: this.onParserImgUrl(node.attributes['src']),
  ...
  TextStyle parseStyle(String style) {
    String _style = '{$style}';
    // print(_style);
    _style = _style
    .replaceAll(' ', '')
    ...
    .replaceAll(':', '":"')
    .replaceAll(';', '","');
    if(_style.lastIndexOf(',"') == _style.length - 2)
      _style = _style.substring(0, _style.length - 3);
    TextStyle result = TextStyle();
    Map map = jsonDecode(_style);
    if(map.containsKey('color'))
    {
      String color = map['color'];
      if(color.indexOf('#') >= 0) {
        result = result.merge(TextStyle(color: Color(0xff000000 + int.parse(color.substring(1)))));
      } 
      else if(color.indexOf('rgb') >= 0) {
        List<String> _rgbs = color.replaceAll('rgb(', '').replaceAll(')', '').split(',');
        result = result.merge(TextStyle(color: Color.fromARGB(255, int.parse(_rgbs[0]), int.parse(_rgbs[1]), int.parse(_rgbs[2]))));
      }
    } 
    if(map.containsKey('background-color')) {
      String color = map['background-color'];
      if(color.indexOf('#') >= 0) {
        result = result.merge(TextStyle(backgroundColor: Color(0xff000000 + int.parse(color.substring(1)))));
      } 
      else if(color.indexOf('rgb') >= 0) {
        List<String> _rgbs = ['0','0','0'];
        if ((color?.contains('rgba(')) ?? false) {
          _rgbs = color.replaceAll('rgba(', '').replaceAll(')', '').split(',');
        }
        if ((color?.contains('rgb(')) ?? false) {
          _rgbs = color.replaceAll('rgb(', '').replaceAll(')', '').split(',');
        }
        result = result.merge(TextStyle(backgroundColor: Color.fromARGB(255, int.parse(_rgbs[0]), int.parse(_rgbs[0]), int.parse(_rgbs[0]))));
      }
    }
    return result;
  }
