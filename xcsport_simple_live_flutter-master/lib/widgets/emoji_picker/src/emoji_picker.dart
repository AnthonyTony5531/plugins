...
import 'package:fast_ai/widgets/emoji_picker/src/recent_emoji.dart';
...
class _EmojiPickerState extends State<EmojiPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return widget.customWidget == null
        ...
        : widget.customWidget?.call(widget.config, state);
  }
  ...
  Future<void> _updateEmojis() async {
    ...
    categoryEmoji.addAll([
      CategoryEmoji(Category.SMILEYS,
          await _getAvailableEmojisUnicode(emoji_list.wechatEmojis, title: 'smileys')),
      // CategoryEmoji(Category.SMILEYS,
  ...
  Future<List<Emoji>> _getAvailableEmojisUnicode(Map<String, int> map,
      {@required String title}) async {
    Map<String,String> emojiMap = map.map((key, value) => MapEntry(key, String.fromCharCode(value)));
    ...
    if (newMap == null) {
      ...
      newMap = await _getPlatformAvailableEmoji(emojiMap);
