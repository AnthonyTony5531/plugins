...
enum AiBottomNavigationBarType {
...
/// static const TextStyle optionStyle = AiTextStyle(fontSize: 30, fontWeight: FontWeight.bold);
...
class AiBottomNavigationBar extends StatefulWidget {
  ...
  ///
  /// The [showSelectedLabels] argument must not be non-null.AiBottomNavigationBarItem
  ///
  /// The [showUnselectedLabels] argument defaults to `true` if [type] is
  /// [BottomNavigationBarType.fixed] and `false` if [type] is
  /// [BottomNavigationBarType.shifting].
  AiBottomNavigationBar({
    Key key,
    @required this.items,
    this.onTap,
    this.currentIndex = 0,
    this.elevation = 8.0,
    AiBottomNavigationBarType type,
    Color fixedColor,
    ...
    bool showUnselectedLabels,
        ...
        assert(
        items.every((AiBottomNavigationBarItem item) => item.title != null) == true,
  ...
  final List<AiBottomNavigationBarItem> items;
  ...
  final AiBottomNavigationBarType type;
  ...
  static AiBottomNavigationBarType _type(
      AiBottomNavigationBarType type,
      List<AiBottomNavigationBarItem> items,
      ) {
    if (type != null) {
      return type;
    }
    return items.length <= 3 ? AiBottomNavigationBarType.fixed : AiBottomNavigationBarType.shifting;
  }
  ...
  static bool _defaultShowUnselected(AiBottomNavigationBarType type) {
    switch (type) {
      case AiBottomNavigationBarType.shifting:
        return false;
      case AiBottomNavigationBarType.fixed:
        return true;
    }
    assert(false);
    return false;
...
class _BottomNavigationTile extends StatelessWidget {
  ...
  final AiBottomNavigationBarType type;
  final AiBottomNavigationBarItem item;
                      ...
                      children: <Widget>[
                        ...
                        Align(
                          child: item.number <= 0
                              ...
                              child: TextHelper.build("${item.number>99?"99+":item.number}",
                                textColor: Colors.white.value, fontSize: item.number>99?6:8,)),
                        )
...
class _BottomNavigationBarState extends State<AiBottomNavigationBar> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(AiBottomNavigationBar oldWidget) {
  ...
  static TextStyle _effectiveAiTextStyle(TextStyle textStyle, double fontSize) {
  ...
  List<Widget> _createTiles() {
    ...
    _effectiveAiTextStyle(widget.selectedLabelStyle, widget.selectedFontSize);
    final TextStyle effectiveUnselectedLabelStyle =
    _effectiveAiTextStyle(widget.unselectedLabelStyle, widget.unselectedFontSize);
...
class AiBottomNavigationBarItem {
  ...
  const AiBottomNavigationBarItem({
    @required this.icon,
    this.title,
    this.number,
    Widget activeIcon,
