...
/// 播放器控制面板
FijkPanelWidgetBuilder fijkControllerBuilder({
  Key key,
  ...
  final bool showController = true,
  final FijkStateBuilder errorBuilder,
  final FijkStateBuilder pauseBuilder,
  final FijkStateBuilder completeBuilder,
}) {
  return (FijkPlayer player, FijkData data, BuildContext context, Size viewSize, Rect texturePos) {
    if (!showController) return const SizedBox.shrink();
    return _FijkControllerView(
      key: key,
      player: player,
      data: data,
      onBack: onBack,
      ...
      completeBuilder: completeBuilder,
    );
  };
}

typedef FijkStateBuilder = Widget Function(BuildContext context, VoidCallback onTap);

class _FijkControllerView extends StatefulWidget {
  ...
  @override
  __FijkControllerState createState() => __FijkControllerState();
}

class __FijkControllerState extends State<_FijkControllerView> {
  ...
  void initState() {
    ...
    if (widget.data.contains('__fijkview_panel_sekto_position')) {
      var pos = widget.data.getValue('__fijkview_panel_sekto_position') as double;
  ...
  void onVerticalDragStartFun(DragStartDetails d) {
    if (d.localPosition.dx > panelWidth() / 2) {
      ...
      FijkVolume.getVol().then((v) {
        if (!widget.data.contains('__fijkview_panel_init_volume')) {
      ...
      FijkPlugin.screenBrightness().then((v) {
        if (!widget.data.contains('__fijkview_panel_init_brightness')) {
  ...
  Widget buildErrorState(BuildContext context) {
    if (widget.errorBuilder != null) {
      return widget.errorBuilder(context, () async {
        await player.reset();
        player.setDataSource(player.dataSource, autoPlay: true);
      });
    }
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('Video Failed To Load', style: TextStyle(color: Colors.white, fontSize: 14.0)),
  ...
  // 暂停播放状态
  Container buildPauseState(BuildContext context) {
    if (widget.pauseBuilder != null) {
      return widget.pauseBuilder(context, () => player.start());
    }
    return Container(
      alignment: Alignment.center,
      child: GestureDetector(
  ...
  //  播放完成状态
  Container buildCompleteState(BuildContext context) {
    if (widget.completeBuilder != null) {
      return widget.completeBuilder(context, () => player.start());
    }
    return Container(
      alignment: Alignment.center,
      child: GestureDetector(
  ...
  Widget buildStateless() {
    ...
    if (volume != null || brightness != null) {
      Widget toast =
          volume == null ? defaultFijkBrightnessToast(brightness, _valController.stream) : defaultFijkVolumeToast(volume, _valController.stream);
  ...
  Widget build(BuildContext context) {
    ...
    // 新增不能自动播放状态下，显示播放按钮
    if((player.state == FijkState.idle || player.state == FijkState.initialized) && !(widget.autoPlay ?? true)){
      ws.add(buildIdleState(context));
    }

    if (player.state == FijkState.error) {
      ws.add(buildErrorState(context));
    }

    if (player.state == FijkState.paused) {
      ws.add(buildPauseState(context));
    }

    if (player.state == FijkState.completed) {
      ws.add(buildCompleteState(context));
    }
