import 'package:common_component/mvvm/viewmodel/stateful_viewmodel.dart';
import 'package:common_component/mvvm/viewmodel/viewmodel.dart';
import 'package:fast_ai/module/common/util/empty_util.dart';
import 'package:fast_ai/pages/rank/anchor_rank/anchor_rank_page_view_model.dart';
//import 'package:fast_ai/pages/rank/ba_rank_utils.dart';
import 'package:flutter/material.dart';

import 'anchor_rank_before_view_style.dart';

///生命周期onAttach->onInit->onDetach->onDispose. refresh不属于生命周期，外面掉用，外部没调用可以不实现。
//其中onAttach, onDetach属于视图的生命周期期间同步方法
class AnchorRankBeforeViewModel
    extends StatefulViewModel<AnchorRankBeforeViewStyle> {
  String name;
  String valueString; //
  String defaultString; // 魅力值 豪气值 粉丝盈利额
  String userIcon; // 头像
  String bgImg; // 背景图片
  bool isPlaying;
  AnchorTitleType titleType;
  bool isWan;
  double bgWidth; // 背景图片大小
  double bgHeight;
  double imgWidth; // 头像大小
  double imgHeight;

  String anchorId;
  String playerId;
  bool isDetailPage; // 是否在详情里面



  AnchorRankBeforeViewModel();

  //参数传给view时候，在这初始化统一入口初始化，自己要啥扩展参数。注意。Page和Dialog.page参数来源于View的构造函数，
  AnchorRankBeforeViewModel.create({
    this.name,
    this.valueString,
    @required this.defaultString,
    this.userIcon,
    this.isPlaying,
    this.isDetailPage = false,
    @required this.bgImg,
    @required this.titleType,
    @required this.bgWidth,
    @required this.bgHeight,
    @required this.imgWidth,
    @required this.imgHeight,
    @required this.isWan,
    this.anchorId,
    this.playerId,
  });

  //更新统一走这个方法
  void update({
    String name,
    String valueString,
    String userIcon,
    bool isPlaying,
    bool isWan,
    String anchorId,
    String playerId,
  }) {
    this.name = name ?? this.name;
    this.userIcon = userIcon ?? this.userIcon;
    this.isPlaying = isPlaying ?? this.isPlaying;
    this.valueString = valueString ?? this.valueString;
    this.isWan = isWan ?? this.isWan;
    this.anchorId = anchorId ?? this.anchorId;
    this.playerId = playerId ?? this.playerId;
  }

  //initState时候同步调用，同步方法。这里不能做io, cache读写.查询等耗时操作。
  @override
  void onAttach() {
  }

  //返回true会调用notifyStateChanged, model变化时候也会重新执行。
  @override
  Future<bool> onInit() async {
    return false;
  }

  //对应widget didUpdateWidget执行后，这个方法会执行，并且oldeViewModel会在这之前执行onDispose。通常在这判断model有没有变动，获取之前的状态做处理。
  @override
  void didModelUpdate(IViewModel o, IViewModel n) {
    AnchorRankBeforeViewModel oldeViewModel = o as AnchorRankBeforeViewModel;
    AnchorRankBeforeViewModel newViewModel = n as AnchorRankBeforeViewModel;
  }

  //view 摧毁时候会调用，或者model变更被移除的时候也会被执行
  @override
  void onDispose() {}

  //这是给外面掉用的
  @override
  Future<dynamic> refresh([Map<String, dynamic> args]) {
    notifyStateChanged();
    return super.refresh();
  }

  @override
  List<AnchorRankBeforeViewStyle> get styleList =>
      [AnchorRankBeforeViewStyle()];
}
