...
class VMServiceWrapper {
  ...
  static final VMServiceWrapper _instance = VMServiceWrapper._privateConstructor();
  ...
  Future<Isolate> getIsolateList() async {
    return await service.getIsolate(main.id);
  }
...
class ExtensionService {
  ...
  Future<String> get serviceStreamName async =>
      (await isProtocolVersionSupported(supportedVersion: SemanticVersion(major: 3, minor: 22))) ? 'Service' : '_Service';
  ...
  Future<Version> getDartIOVersion(String isolateId) => serviceClient.getDartIOVersion(isolateId);
  ...
  Future<Response> callMethod(String method) {
    if (registeredMethodsForService.containsKey(method)) {
      return (serviceClient.callMethod(registeredMethodsForService[method].last, isolateId: main.id));
    }
