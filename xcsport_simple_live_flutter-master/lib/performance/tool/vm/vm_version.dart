...
class FlutterVersion extends SemanticVersion {
  FlutterVersion._({
    ...
    final List<String> _versionParts =
        version.split('.').map((String part) => RegExp(r'\d+').stringMatch(part) ?? '0').toList();
    major = _versionParts.isNotEmpty ? int.tryParse(_versionParts.first) ?? 0 : 0;
  ...
  String get frameworkVersionSummary => 'revision $frameworkRevision • $frameworkCommitDate';
...
class SemanticVersion implements Comparable<SemanticVersion> {
  ...
  bool isSupported({@required SemanticVersion supportedVersion}) => compareTo(supportedVersion) >= 0;
