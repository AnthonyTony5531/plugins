...
- (void)initDemoUI {
    ...
    NSLog(@"展示/旗舰/国际/CN2版 isShowNewLogin=%d", self.isShowNewLogin);
#endif
...
#ifdef kDEV_FORCE_USE_LOCAL_ASSETS
        self.isLocalNetAssets = YES;
#endif
        [self initUI];
        [self querySDKConfig];
    }
...
- (void)configlLogin {
    ...
    [self.loginView setEntrySDKBlock:^(NSUInteger index, NSString * _Nonnull account, NSString * _Nonnull password, NSInteger appType) {
        ...
        [XCSDKUtils loginWithType:envType
            ...
            if (!config) {
                [weakSelf showHUD:@"模拟配置获取失败" afterDelay:1];
                return ;
            }
    ...
    self.loginView.tryPlay = ^{
        ...
        [XCSDKUtils queryQuickLogin:envType handler:^(NSDictionary * _Nullable resp, NSError * _Nullable error) {
            ...
            if (resp[@"data"][@"token"]) {
                ...
                [weakSelf hideHUD:error.localizedDescription];
            }
        }];
    };
}
...
#ifdef kDEV_FORCE_USE_LOCAL_ASSETS
            self.isLocalNetAssets = [dict[@"title"] isEqualToString:@"DEV"];
#endif
        }];
        return NO;
    }
